    ...
    event_schema: {
      ...
      options: {
        sql: |||
          ...
          where table_schema = current_schema and table_name not like '%~%' and table_name not like '$%' and table_name != '_users' and column_name not like '$%' and column_name not in ('_user', '_time', '$server_time')
          group by 1
        |||,
      },
    },
